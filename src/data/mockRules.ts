...
if (input.unit === "C") {
  ...
  {
    id: "rule-003",
    name: "Нормализация уровня нефти",
    parameterType: "Уровень нефти",
    status: "draft",
    version: 1,
    author: "Смирнов И.И.",
    lastCheck: "2024-02-13 16:45",
    reportsUsed: 0,
    parametersLinked: 56,
    errorCount: 0,
    warningCount: 1,
    code: `// Нормализация уровня нефти к стандартным условиям
const refTemp = 15; // °C
const alpha = 0.00065; // коэффициент объёмного расширения
output.value = input.value * (1 - alpha * (input.temperature - refTemp));
output.unit = "м³";`,
    createdAt: "2024-02-01",
    category: "Уровень нефти",
  },
  {
    id: "rule-004",
...
if (input.value < MIN_PRESSURE || input.value > MAX_PRESSURE) {
  ...
  {
    id: "rule-005",
    name: "Коррекция плотности",
    parameterType: "Плотность",
    status: "active",
    version: 4,
    author: "Петров К.С.",
    lastCheck: "2024-02-14 15:20",
    reportsUsed: 18,
    parametersLinked: 134,
    errorCount: 0,
    warningCount: 0,
    code: `// Коррекция плотности по температуре
const rho15 = input.value;
const T = input.temperature;
const beta = 0.000612;
output.value = rho15 * (1 - beta * (T - 15));
output.unit = "кг/м³";`,
...
if (input.value < 0) {
  ...
  {
    id: "rule-009",
    name: "Пересчет расхода",
    parameterType: "Расход",
    status: "draft",
    version: 1,
    author: "Смирнов И.И.",
    lastCheck: "2024-02-11 17:45",
    reportsUsed: 0,
    parametersLinked: 42,
    errorCount: 1,
    warningCount: 3,
    code: `// Пересчет расхода из м³/ч в л/мин
if (input.unit === "m3/h") {
    output.value = input.value * 1000 / 60;
    output.unit = "л/мин";
} else if (input.unit === "gpm") {
